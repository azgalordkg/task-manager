export const RED_COLORS = {
  FIRST: '#FA752B',
  SECOND: '#DE4D26',
  THIRD: '#F44336',
  FORTH: '#DE2658',
  FIFTH: '#FA2BCB',
};

export const PURPLE_COLORS = {
  FIRST: '#FA2B77',
  SECOND: '#DE26C5',
  THIRD: '#CE36F4',
  FOURTH: '#8726DE',
  FIFTH: '#5F2BFA',
};

export const BLUE_COLORS = {
  FIRST: '#0D4BFF',
  SECOND: '#0C7EE8',
  THIRD: '#00C8FF',
  FORTH: '#0CE8DD',
  FIFTH: '#0DFFAE',
};

export const GREEN_COLORS = {
  FIRST: '#2BFAF3',
  SECOND: '#26DEA3',
  THIRD: '#36F482',
  FORTH: '#26DE3B',
  FIFTH: '#55FA2B',
};

export const YELLOW_COLORS = {
  FIRST: '#FFEE30',
  SECOND: '#E8C62C',
  THIRD: '#FFC93D',
  FORTH: '#E8A02C',
  FIFTH: '#FF9530',
};

export const COLORS = {
  WHITE_LIGHT: '#FFFFFF',
  WHITE_MEDIUM: '#F5F5F5',
  WHITE_DARK: '#b9b9b9',
  GREY_LIGHT: '#6a6a6a',
  GREY_MEDIUM: '#3F3F3F',
  GREY_DARK: '#353535',
  BLACK_MEDIUM: '#2A2A2A',
  BLACK_DARK: '#1F1F1F',
  GREEN: '#3DB39E',
  RED: RED_COLORS.THIRD,
  YELLOW: YELLOW_COLORS.THIRD,
  BLUE: BLUE_COLORS.THIRD,
};

export const LIGHT_SCHEMA = {
  // Background
  BACKGROUND_PRIMARY: COLORS.BLACK_DARK,
  BACKGROUND_SECONDARY: COLORS.BLACK_MEDIUM,
  BACKGROUND_ACCENT: COLORS.YELLOW,
  BACKGROUND_NEUTRAL: COLORS.WHITE_LIGHT,
  // Text
  TEXT_PRIMARY: COLORS.WHITE_LIGHT,
  TEXT_SECONDARY: COLORS.WHITE_DARK,
  TEXT_INFO: COLORS.GREY_LIGHT,
  // Buttons
  BUTTONS_PRIMARY: COLORS.GREEN,
  BUTTONS_SECONDARY: COLORS.WHITE_MEDIUM,
  BUTTONS_ACCENT: COLORS.BLUE,
  BUTTONS_NEUTRAL: COLORS.GREY_MEDIUM,
  BUTTONS_ACTION: COLORS.WHITE_LIGHT,
};

export const DARK_SCHEMA = {
  ...LIGHT_SCHEMA,
};

export const DARK_COLORS = {
  WHITE_LIGHT: '#FFFFFF',
  WHITE_MEDIUM: '#F5F5F5',
  WHITE_DARK: '#b9b9b9',
  GREY_LIGHT: '#6a6a6a',
  GREY_MEDIUM: '#3F3F3F',
  GREY_DARK: '#353535',
  BLACK_MEDIUM: '#2A2A2A',
  BLACK_DARK: '#1F1F1F',
  GREEN: '#3DB39E',
  RED: RED_COLORS.THIRD,
  YELLOW: YELLOW_COLORS.THIRD,
  BLUE: BLUE_COLORS.THIRD,
};

export const AVAILABLE_COLORS = [
  ...Object.values(YELLOW_COLORS),
  ...Object.values(RED_COLORS),
  ...Object.values(PURPLE_COLORS),
  ...Object.values(BLUE_COLORS),
  ...Object.values(GREEN_COLORS),
];
